...
for (dist in distributions) {
    ...
    for (model_name in names(models)) {
      ...
      tryCatch({
        ...
        results[[model_name]] <- list(
          fit = fit,
          kolmogrov_test = ks.test(prob[test_ind], p)$p.value,
          true_mse = mean((p - prob[test_ind])^2),
          ll_p = log_loss(prob[test_ind], p),
          ll_pi = log_loss(p, p),
          true_ll_epistemic_loss = log_loss(prob[test_ind], p) - log_loss(prob[test_ind], prob[test_ind]),
          ...
          bs_epistemic_loss = brier_score(prob[test_ind], p) - brier_score(prob[test_ind], prob[test_ind]),
          ...
          ece_stat = ece_mce(test$y, p, g = 10, 'C')$ece,
          ece_acc =  ece_mce(test$y, p, g=10, 'C',yhat = y_pred)$ece_acc
        )
        cat(" success\n")
      }, error = function(e) {
  ...
  if (length(all_metrics) > 0) {
    ...
    write.csv(pval_summary, paste0("calibration/pvalue_counts_", dist$name, ".csv"), row.names = FALSE)
    print(pval_summary)
  }
}
